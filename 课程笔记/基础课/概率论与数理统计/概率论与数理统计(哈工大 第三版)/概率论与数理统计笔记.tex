...
\setstretch{1.15}             % 行间距适中
\setlength{\parskip}{0.4em}   % 段落间距：不大不小，最耐看
\setlength{\parindent}{0pt}   % 无缩进
\frenchspacing                % 杜绝标题/文字被强行撑开
...
% 定理环境 —— 干净紧凑
\newtheoremstyle{mytheorem}{4pt}{5pt}{\normalfont}{0pt}{\bfseries}{：}{0.3em}{}
...
\setlist{leftmargin=2em, itemsep=0.25em, parsep=0pt, topsep=0.3em}
% 新增：专门用于最大似然估计的列表环境，保证换行内容对齐
...
\titleformat{\section}
  ...
  {0.3em}  % 标号和文字贴紧
  {}
\titlespacing*{\section}{0pt}{1.0em}{0.6em} % 标题上下干净

% 关键优化：subsection强制左对齐，解决两字标题分散问题
...
% 目录
\titlecontents{section}[0pt]{\bfseries}{\thecontentslabel\quad}{}{\ \titlerule*[0.5pc]{.}\contentspage}
\titlecontents{subsection}[1.5em]{}{\thecontentslabel\quad}{}{\ \titlerule*[0.5pc]{.}\contentspage}
...
\newcommand{\makeCover}
{
  \begin{titlepage}
    ...
    % 顶部名言（贴合概率论主题，斜体+引用样式）
    {\Large \itshape “概率是生活的真正指南。” —— 约瑟夫·巴特勒}\\[0.5cm]
    \rule{\textwidth}{1pt}\\[0.5cm]
    
    % 主标题
    {\Huge \bfseries \color{darkblue} 概率论与数理统计}\\[8pt]
    {\Huge \bfseries \color{darkblue} 核心公式整理}\\[12pt]
    ...
    % 学科说明（按要求修改）
    {\Large \textbf{内容概要：} 随机变量 | 数字特征 | 抽样分布 | 参数估计}\\[8pt]
    {\Large \textbf{适用场景：} 课程复习 · 公式速查}\\[1.5cm]
    
    % 整理者信息
    {\Large \textbf{整理者：}Duckweed-yhb}\\[6pt]
    {\large \textbf{个人博客：}\url{https://duckweed-yhb.github.io/}}\\[6pt]
    ...
    {\small \itshape Copyright © \the\year\ Duckweed-yhb · 禁止商用 · 欢迎交流}\\[0.5cm]
...
\begin{property}[正态分布绝对值的期望公式]
若 $Y \sim N(0, \tau^2)$，则其绝对值的期望为：
$$
\E|Y| = \tau \sqrt{\frac{2}{\pi}}
$$
其中 $\tau$ 为正态分布的标准差。

\symnote{应用示例：若 $X_i - \overline{X} \sim N\left(0, \frac{n-1}{n}\sigma^2\right)$，则
$$
\E|X_i - \overline{X}| = \sigma\sqrt{\frac{n-1}{n}} \cdot \sqrt{\frac{2}{\pi}} = \sigma\sqrt{\frac{2(n-1)}{n\pi}}
$$
}
\end{property}
...
1. 有界性：$0 \le F(x,y) \le 1$；

2. 单调不减性：$x_1 < x_2 \implies F(x_1,y) \le F(x_2,y)$，$y_1 < y_2 \implies F(x,y_1) \le F(x,y_2)$；

3. 极限性质：$F(-\infty,y)=F(x,-\infty)=F(-\infty,-\infty)=0, F(+\infty,+\infty)=1$；

4. 右连续性：$F(x,y) = F(x^+,y) = F(x,y^+)$；

5. 非负性：$x_1 \le x_2, y_1 \le y_2$ 时，$F(x_2,y_2) - F(x_2,y_1) - F(x_1,y_2) + F(x_1,y_1) \ge 0$。
\end{property}
...
3. 线性性：$\E\left(\sum_{i=1}^n X_i\right) = \sum_{i=1}^n \E(X_i)$（无需独立）；

4. 独立乘积：若 $X_1,\dots,X_n$ 独立，则 $\E\left(\prod_{i=1}^n X_i\right) = \prod_{i=1}^n \E(X_i)$。
\symnote{$\prod$：乘积运算符。}
\end{property}
...
3. 独立和：若 $X_1,\dots,X_n$ 独立，则 $\D\left(\sum_{i=1}^n X_i\right) = \sum_{i=1}^n \D(X_i)$；

4. 一般线性组合：对任意常数 $a,b$，有 $\D(aX + bY) = a^2\D(X) + b^2\D(Y) + 2ab\Cov(X,Y)$；

5. $\D(XY) = \D(X)\D(Y) + \D(X)\left[\E(Y)\right]^2 + \D(Y)\left[\E(X)\right]^2$（$X \indep Y$）；

6. $\D(X) = 0 \iff \Prob(X = \E(X)) = 1$。
\symnote{$\iff$：充要条件。}
\end{property}
...
2. 数乘：$\Cov(aX, bY) = ab\Cov(X, Y)$；

3. 可加性：$\Cov(X_1 + X_2, Y) = \Cov(X_1, Y) + \Cov(X_2, Y)$。
\end{property}
...
2. $|\rho_{XY}| = 1 \iff \Prob(Y = a + bX) = 1$（线性相关）；

3. 不相关等价：$\Cov(X,Y)=0 \iff \D(X+Y)=\D(X)+\D(Y) \iff \E(XY)=\E(X)\E(Y)$。
\end{property}
...
1. 可加性：$X \sim \chi^2(m), Y \sim \chi^2(n)$ 独立，则 $X+Y \sim \chi^2(m+n)$；

2. 期望：$\E[Y] = n$；方差：$\D(Y) = 2n$。
...
若 $X \sim \chi^2(n_1), Y \sim \text{chi}^2(n_2)$ 且独立，则
$$
...
1. 样本 $k$ 阶原点矩：$A_k = \frac{1}{n} \sum_{i=1}^n X_i^k$；

2. 步骤：总体矩 $\alpha_k = q_k(\theta_1,\dots,\theta_m) \implies$ 解出 $\theta_j = h_j(\alpha_1,\dots,\alpha_m) \implies$ 替换为 $\hat{\theta}_j = h_j(A_1,\dots,A_m)$。
...
\[
\begin{split}
\text{连续型：} &\quad L(\theta) = \prod_{i=1}^{n} f(x_i;\theta), \\
\text{离散型：} &\quad L(\theta) = \prod_{i=1}^{n} \Prob(X = x_i;\theta);
\end{split}
\]
...
1. $\sigma^2$ 已知，$\mu$ 的置信区间：$\left( \samplemean{x}-\ualpha \frac{\sigma}{\sqrt{n}},\; \samplemean{x}+\ualpha \frac{\sigma}{\sqrt{n}} \right)$；

2. $\sigma^2$ 未知，$\mu$ 的置信区间：$\left( \samplemean{x}-\talpha{n-1}\frac{s}{\sqrt{n}},\; \samplemean{x}+\talpha{n-1}\frac{s}{\sqrt{n}} \right)$；

3. $\sigma^2$ 的置信区间：$\left( \frac{(n-1)s^2}{\xalpha{n-1}},\; \frac{(n-1)s^2}{\xalphaone{n-1}} \right)$。
\symnote{$\ualpha$：标准正态分位数；$\talpha{n-1}$：t分布分位数；$\xalpha{n-1}$：卡方分布分位数。}
\end{property}

\section{拓展：伽马函数}
\subsection{伽马函数的定义与性质}
\begin{definition}[伽马函数]
对 $\alpha > 0$，定义 $\Gamma(\alpha) = \int_{0}^{+\infty} x^{\alpha-1} \e^{-x} \dif x$，核心性质：
1. 递推公式：$\Gamma(\alpha+1) = \alpha \Gamma(\alpha)$；

2. 特殊值：$\Gamma(1)=1, \Gamma\left(\frac{1}{2}\right)=\sqrt{\pi}, \Gamma(n+1)=n!$；

3. 应用：伽马分布 $Ga(\alpha,\beta)$ 含 $\Gamma(\alpha)$，$\chi^2(n) \sim Ga\left(\frac{n}{2},\frac{1}{2}\right)$。
\symnote{$\Gamma(\alpha)$：伽马函数；$n!$：阶乘；$Ga(\alpha,\beta)$：伽马分布。}
\end{definition}

\section{例题索引}
\subsection{概率论综合训练题单}
